...
const createEvent = async (req, res) => {
  try {
    ...
    if (req.user.role !== "admin" && req.user.role !== "organizer") {
      return res
        .status(403)
        .json({ message: "Non autorisé. Rôle admin ou organizer requis." });
    }
...
const updateEvent = async (req, res) => {
  try {
    ...
    if (
      ...
      return res
        .status(403)
        .json({ message: "Non autorisé à modifier cet événement" });
    }
...
const deleteEvent = async (req, res) => {
  try {
    ...
    if (
      ...
      return res
        .status(403)
        .json({ message: "Non autorisé à supprimer cet événement" });
    }
    ...
    res.json({
      message: "Événement et tickets associés supprimés avec succès",
    });
  } catch (error) {
